...
class SupportStageDetails(object):
    ...
    def __init__(self, provider_stage=None, per_storage=None):  # noqa: E501
        """SupportStageDetails - a model defined in Swagger"""  # noqa: E501
        self._provider_stage = None
        self._per_storage = None
        self.discriminator = None
        if provider_stage is not None:
            self.provider_stage = provider_stage
        if per_storage is not None:
            self.per_storage = per_storage

    @property
    def provider_stage(self):
        """Gets the provider_stage of this SupportStageDetails.  # noqa: E501

        Current support stage of the provider - depends on the stages of its storages: * `joining` - the provider has freshly supported the space and is catching up   with dbsync changes from other providers  * `active` - the provider is operational and up-to-date with other providers,   all storages are in 'active' stage  * `remodelling` - the provider is undergoing changes concerning its storage(s)   that support the space, e.g. a storage has been added or removed   and is not yet in active stage (such modifications may require   long-lasting data transfers between storages).  * `evicting_replicas` (unsupporting) - the provider has chosen to cease support   for the space with all of its storages and is now safely migrating   data to other providers  * `purging_storages` (unsupporting) - the provider has evicted all replicas and   is now purging its storages from support remnants  * `purging_database` (unsupporting) - the provider purged its storages and is   now purging the database (including file metadata, locations etc)  * `retired` - the provider has supported the space in the past, but not anymore   # noqa: E501

        :return: The provider_stage of this SupportStageDetails.  # noqa: E501
        :rtype: str
        """
        return self._provider_stage

    @provider_stage.setter
    def provider_stage(self, provider_stage):
        """Sets the provider_stage of this SupportStageDetails.

        Current support stage of the provider - depends on the stages of its storages: * `joining` - the provider has freshly supported the space and is catching up   with dbsync changes from other providers  * `active` - the provider is operational and up-to-date with other providers,   all storages are in 'active' stage  * `remodelling` - the provider is undergoing changes concerning its storage(s)   that support the space, e.g. a storage has been added or removed   and is not yet in active stage (such modifications may require   long-lasting data transfers between storages).  * `evicting_replicas` (unsupporting) - the provider has chosen to cease support   for the space with all of its storages and is now safely migrating   data to other providers  * `purging_storages` (unsupporting) - the provider has evicted all replicas and   is now purging its storages from support remnants  * `purging_database` (unsupporting) - the provider purged its storages and is   now purging the database (including file metadata, locations etc)  * `retired` - the provider has supported the space in the past, but not anymore   # noqa: E501

        :param provider_stage: The provider_stage of this SupportStageDetails.  # noqa: E501
        :type: str
        """
        allowed_values = ["joining", "active", "remodelling", "evicting_replicas", "purging_storages", "purging_database", "retired"]  # noqa: E501
        if provider_stage not in allowed_values:
            raise ValueError(
                "Invalid value for `provider_stage` ({0}), must be one of {1}"  # noqa: E501
                .format(provider_stage, allowed_values)
            )

        self._provider_stage = provider_stage

    @property
    def per_storage(self):
        """Gets the per_storage of this SupportStageDetails.  # noqa: E501

        The map of storage Ids of the provider that support this space and their corresponding support stages.   # noqa: E501

        :return: The per_storage of this SupportStageDetails.  # noqa: E501
        :rtype: dict(str, str)
        """
        return self._per_storage

    @per_storage.setter
    def per_storage(self, per_storage):
        """Sets the per_storage of this SupportStageDetails.

        The map of storage Ids of the provider that support this space and their corresponding support stages.   # noqa: E501

        :param per_storage: The per_storage of this SupportStageDetails.  # noqa: E501
        :type: dict(str, str)
        """

        self._per_storage = per_storage
    ...
    def to_dict(self):
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            ...
            if isinstance(value, list):
                ...
                result[attr] = value
        if issubclass(SupportStageDetails, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, SupportStageDetails):
            return False
